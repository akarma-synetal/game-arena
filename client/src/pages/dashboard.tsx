...
export default function Dashboard() {
  ...
  const upcomingTournaments =
    tournaments
      ?.filter(
        (t: any) =>
          t.status === "upcoming" || t.status === "registration_opened",
      )
      .slice(0, 3) || [];
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-card border border-white/5 p-8 rounded-xl relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
          ...
          <div className="text-center md:text-left">
            <h2 className="text-muted-foreground font-display tracking-[0.3em] text-[10px] uppercase mb-1">
            ...
            <div className="flex flex-wrap gap-2 justify-center md:justify-start">
              <NeonBadge color="primary">
                {profile?.subscriptionTier?.toUpperCase() || "FREE"} AGENT
              </NeonBadge>
              <NeonBadge color="accent">
        ...
        <div className="flex flex-col sm:flex-row gap-8 relative z-10 items-center">
          <div className="text-center">
            <p className="text-[10px] text-muted-foreground font-display uppercase tracking-[0.2em] mb-1">
              Deployment XP
            </p>
          ...
          <div className="text-center">
            ...
            <p className="text-3xl font-bold text-secondary text-glow-cyan font-display tracking-tighter">
              #4,209
            </p>
          </div>
          <div className="w-px bg-white/10 h-10 hidden sm:block" />
          <div className="text-center">
            ...
            <p className="text-3xl font-bold text-primary text-glow font-display tracking-tighter">
              ${profile?.winnings || 0}
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
            ...
            <CyberCard glowColor="accent">
              ...
              <h3 className="text-3xl font-bold font-display tracking-widest uppercase text-glow-purple">
        ...
        <div className="space-y-6">
          <CyberCard glowColor="accent" className="p-0 overflow-hidden">
            <div className="p-4 border-b border-white/5 bg-white/5 flex items-center justify-between">
              ...
              <span className="text-[8px] text-accent animate-pulse">
                LIVE_SYNC
              </span>
            </div>
            <div className="p-4 space-y-4">
              {tourneyLoading ? (
                <p className="text-[10px] uppercase text-muted-foreground animate-pulse text-center py-4">
                  Scanning Sector...
                </p>
                ...
                upcomingTournaments.map((t: any) => (
                  <div
                    key={t.id}
                    className="group flex items-center justify-between p-3 bg-black/40 border border-white/5 rounded transition-all hover:border-accent/50 cursor-pointer"
                  >
                    <div className="flex flex-col">
                      <span className="text-[10px] text-accent font-display uppercase tracking-tighter mb-1">
                        {t.type}
                      </span>
                      <h4 className="font-bold text-xs truncate max-w-[120px] uppercase tracking-wide">
                        {t.name}
                      </h4>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-[8px] text-muted-foreground uppercase tracking-widest">
                        {format(new Date(t.startDate), "HH:mm")}
                      </p>
                    </div>
                  </div>
                ))
            ...
            <div className="p-4 space-y-3">
              {teamsLoading ? (
                ...
                teams.map((t: any) => (
                  <div
                    key={t.id}
                    className="flex items-center gap-3 p-3 hover:bg-white/5 rounded transition-all cursor-pointer border border-transparent hover:border-white/10 group"
                  >
          ...
          <CyberCard
            glowColor="primary"
            className="p-4 bg-gradient-to-br from-primary/20 to-transparent border-primary/20"
          >
            ...
            <p className="text-[10px] text-muted-foreground mb-4 uppercase leading-relaxed">
              Upgrade to Elite tier for instant mission access and 2x jackpot
              rewards.
            </p>
            <Button className="w-full h-8 bg-primary text-black font-display uppercase tracking-widest text-[10px] hover:bg-primary/80 transition-all border-none">
              Upgrade Now
            </Button>
          </CyberCard>
        </div>
